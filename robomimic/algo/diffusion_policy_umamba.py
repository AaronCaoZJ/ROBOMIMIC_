...
@register_algo_factory_func("diffusion_policy_umamba")
def algo_config_to_class(algo_config):
    ...
    if algo_config.umamba.enabled:
        return DiffusionPolicyUMamba, {}
    elif algo_config.transformer.enabled:
        raise NotImplementedError()
    elif algo_config.unet.enabled:
        return NotImplementedError()
    else:
        raise RuntimeError()

class DiffusionPolicyUMamba(PolicyAlgo):
    ...
    def train_on_batch(self, batch, epoch, validate=False):
        ...
        with TorchUtils.maybe_no_grad(no_grad=validate):
            info = super(DiffusionPolicyUMamba, self).train_on_batch(batch, epoch, validate=validate)
    ...
    def log_info(self, info):
        ...
        """
        log = super(DiffusionPolicyUMamba, self).log_info(info)
...
class ConditionalUnet1D(nn.Module):
    def __init__(self, 
        ...
        #     ),
        # ])
        self.mamba_layer = MambaLayer(dim = mid_dim)
